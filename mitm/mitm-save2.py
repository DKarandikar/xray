...
def openfile():
    ...
    writer = csv.writer(f)
    if newFile:
        writer.writerow(['app', 'company', 'version', 'device', 'platform', 'researcher', 'time', 'type' 'runid', 'host', 'url', 'method', 'headers', 'body'])
    return f,writer
...
def request(flow):
    ...
    f,writer = openfile()
    writer.writerow([app, company, version, device, platform, researcher, int(time.time()*1000), "request", runid, flow.request.pretty_host, urllib.quote(flow.request.url), flow.request.method, urllib.quote(json.dumps(dict(flow.request.headers))), urllib.quote(flow.request.body)]) 
    f.close()
...
def response(flow):
    ...
    f,writer = openfile()
    writer.writerow([app, company, version, device, platform, researcher, int(time.time()*1000), "response", runid, "N/A" , "N/A", "N/A", urllib.quote(json.dumps(dict(flow.response.headers))), urllib.quote(flow.response.body)]) 
    f.close()